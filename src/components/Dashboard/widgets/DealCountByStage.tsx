...
export const DealCountByStage: React.FC<DealCountByStageProps> = ({ data, viewMode }) => {
  ...
  return (
    <div style={{ background: theme.colors.cardBg, padding: theme.spacing['2xl'], borderRadius: theme.borderRadius['2xl'], border: theme.colors.cardBorder }}>
      <h3 style={{ fontSize: theme.fontSize.xl, fontWeight: theme.fontWeight.semibold, marginBottom: theme.spacing.sm, color: theme.colors.text }}>Deal Count by Stage <span style={{ fontSize: theme.fontSize.base, color: theme.colors.textSecondary, fontWeight: theme.fontWeight.normal }}>({isCompanyView ? 'Company' : 'Binee'} View)</span></h3>
      <ResponsiveContainer width="100%" height={320}>
        <BarChart data={data}>
          ...
          <Bar dataKey="count" fill={theme.colors.success} radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
