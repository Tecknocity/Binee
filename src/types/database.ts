...
/**
 * One row per file or image the user has uploaded in a setup conversation.
 * Phase 2 architectural store: bytes/text stored once, structured digest
 * generated by Haiku at upload time, replayed in subsequent system prompts
 * so the model never "forgets" earlier attachments without re-sending the
 * full content every turn.
 */
